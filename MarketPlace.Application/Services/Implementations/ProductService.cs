...
namespace MarketPlace.Application.Services.Implementations
{
    public class ProductService : IProductService
    {
        ...
        public async Task<List<Product>> GetPopularProducts(int take)
        {
            return await _productRepository.GetQuery().AsQueryable()
        ...
        public async Task<FilterProductDTO> FilterProducts(FilterProductDTO filter)
        {
            ...
            if(filter.FilterMinPrice != 0 ) query = query.Where(s => s.Price >= filter.SelectedMinPrice);
            if (filter.FilterMaxPrice != 0) query = query.Where(s => s.Price <= filter.SelectedMaxPrice);
        ...
        public async Task EditCategory(ProductCategory category)
        {
            var mainCategory = await _productCategoryRepository.GetQuery().AsQueryable()
                .SingleOrDefaultAsync(s => s.Id == category.Id);

            mainCategory.Title = category.Title;
            mainCategory.UrlName = category.UrlName;
            mainCategory.CreateDate = category.CreateDate;
            mainCategory.IsActive = true;
            mainCategory.IsDelete = false;  
        ...
        {
            _productCategoryRepository.DeleteEntity(category);
            await _productCategoryRepository.SaveChanges();
        }
